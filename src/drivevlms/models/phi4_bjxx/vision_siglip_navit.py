...
class SiglipVisionEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values: torch.FloatTensor, patch_attention_mask: torch.BoolTensor) -> torch.Tensor:
        ...
        for batch_idx, p_attn_mask in enumerate(patch_attention_mask):
            nb_patches_h = p_attn_mask[:, 0].sum()  # 32，当输入为448*448时，nb_patches_h=nb_patches_w=32
...
class SiglipEncoder(nn.Module):
    ...
    def forward(
        ...
        # gt = torch.load('hidden_states-npuforward.pt')
        # mse = torch.nn.functional.mse_loss(inputs_embeds, gt)
        # print(f'hidden_states feat :{mse}')
        for i, encoder_layer in enumerate(self.layers):
            ...
            #     gt = torch.load('encoderlayer-idx.pt')
            #     mse = torch.nn.functional.mse_loss(hidden_states, gt)
            #     print(f'-2 layer feat :{mse}')
            if output_attentions:
