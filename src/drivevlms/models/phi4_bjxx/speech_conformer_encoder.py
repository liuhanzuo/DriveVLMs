...
class MeanVarianceNormLayer(nn.Module):
    ...
    def __init__(self, input_size):
        super().__init__()
        self.input_size = input_size
        self.register_buffer("global_mean", torch.zeros(input_size))    # 80
        self.register_buffer("global_invstd", torch.ones(input_size))   # 80
...
class NemoConvSubsampling(torch.nn.Module):
    ...
    def forward(self, x, mask):
        ...
        if self.conv2d_subsampling:
            ...
            x = x.flatten(2,3)
            x = self.out(x)
            # x = self.out(x.transpose(1, 2).reshape(b, t, -1))   # Eq(Mod(10240*(((L['xs_pad'].size()[1] - 1)//8)) + 10240, ((L['xs_pad'].size()[1] - 1)//8) + 1), 0)
...
class ConformerEncoder(TransformerEncoderBase):
    ...
    def forward(self, xs_pad, masks):
        ...
        if seq_len > max_seq_len:
            ...
            input_tensor = unfold_tensor(input_tensor, max_seq_len) # (N, T, D) -》 (NT', max_seq_len, D)，T'=T//max_seq_len
            if masks is not None:   # masks None
                ...
                masks_unfold = None
            hs_mask = self.calculate_hs_mask(input_tensor, input_tensor.device, masks_unfold) # calculate hs_mask based on the unfolded pad mask    # 全为1
        layer_emb = None
