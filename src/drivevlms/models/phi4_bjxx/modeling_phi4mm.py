...
class Phi4MMImageEmbedding(nn.Module):
    ...
    def forward(self, input_ids: torch.LongTensor, input_embeds: torch.FloatTensor, image_sizes=None, **kwargs) -> torch.FloatTensor:
        ...
        if image_sizes is None and 'image_sizes' in kwargs:
            ...
            print(f'image size is {image_sizes}')
        ...
        if len(positions.tolist()) > 0:
            if self.use_hd_transform and img_sizes is not None and len(img_sizes):
                ...
                base_feat_height_target = self.base_feat_height_target  # 16
                base_resolution = self.crop_size    # 448
                base_feat_height_reduction = self.base_feat_height_reduction    # 1
...
class Phi4MMMLP(nn.Module):
    ...
    def forward(self, hidden_states: torch.FloatTensor, **kwargs,) -> torch.FloatTensor:
        up_states = self.gate_up_proj(hidden_states, **kwargs,)

        gate, up_states = up_states.chunk(2, dim=-1)
        up_states = up_states * self.activation_fn(gate)

        return self.down_proj(up_states, **kwargs,)
...
class Phi4MMForCausalLM(Phi4MMPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        if num_logits_to_keep is None: num_logits_to_keep=1
        hidden_states = outputs[0]
        # Only compute necessary logits, and do not upcast them to float if we are not computing the loss
        logits = self.lm_head(hidden_states[:, -num_logits_to_keep:, :])
        
        # print(f'num_logits_to_keep:{num_logits_to_keep}') # 1
        # print('_seen_tokens ',past_key_values._seen_tokens)
        loss = None
...
class Phi4MMDomainForCausalLM(Phi4MMPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]
    ...
    def __init__(self, config):
        ...
        # 3. domain lora
        domain_lora_config = vision_lora_config
        peft_model.base_model.active_adapter.append("domain")
        peft_model.add_adapter("domain", domain_lora_config)
