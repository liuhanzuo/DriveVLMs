...
def base_generate(model, tokenizer, input_ids, max_new_tokens=10, 
                  ...
                  early_stop=False,input_mode=None, use_cache=True):
...
def exact_self_speculative_generate(model, tokenizer, input_ids, max_new_tokens=10, early_stop=False,
                 ...
                 do_sample=False, do_sample_draft=False, top_k=0, top_p=0.85, temperature=0.2, input_mode=None, use_cache=True):
        ...
        while True:
            ...
            if step == 0:
                ...
                for step_draft in range(max_step_draft):
                    ...
                    draft_past_key_values = draft_output['past_key_values'] if use_cache else None
...
def self_speculative_sample(model, tokenizer, input_ids, max_new_tokens=10, early_stop=False,
                 ...
                 top_k=0, top_p=0.85, temperature=0.2, input_mode=None, use_cache=True):
...
def infer_input_ids(model, tokenizer, input_ids, generate_fn='base', 
          decode_timing=True, seed=42, input_mode=None, *args, **kargs):
    ...
    if decode_timing:
        tic = time.time()
    generate_dict = generate_fn(model, tokenizer, input_ids, input_mode = input_mode, *args, **kargs)
    generate_ids = generate_dict['generate_ids']
    if decode_timing:
        toc = time.time()
        decode_time = toc - tic
